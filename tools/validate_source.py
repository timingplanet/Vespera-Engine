...
required = [
    ...
    root / "examples/reference_game/managed/GameplayShowcase.cs",
    ...
    root / "editor/editor_state.hpp",
    root / "editor/editor_selection.hpp",
    root / "editor/editor_selection.cpp",
    root / "editor/editor_console.hpp",
    root / "editor/editor_console.cpp",
    root / "editor/editor_history.hpp",
    root / "editor/editor_history.cpp",
    root / "editor/editor_assets.hpp",
    root / "editor/editor_assets.cpp",
    root / "editor/editor_play_controls.hpp",
    root / "editor/editor_play_controls.cpp",
    root / "editor/editor_project_session.hpp",
    root / "editor/editor_project_session.cpp",
    root / "editor/editor_command_runtime.hpp",
    root / "editor/editor_view_helpers.hpp",
    root / "editor/editor_view_helpers.cpp",
    root / "editor/editor_scene_commands.hpp",
    root / "editor/editor_scene_commands.cpp",
    root / "editor/editor_prefabs.hpp",
    root / "editor/editor_prefabs.cpp",
    root / "editor/editor_managed.hpp",
    root / "editor/editor_managed.cpp",
    root / "editor/editor_asset_interactions.hpp",
    root / "editor/editor_asset_interactions.cpp",
    ...
    root / "website/first-game.html",
    root / "website/editor.html",
    root / "website/input.html",
    root / "website/managed-scripting.html",
    root / "website/gameplay-api.html",
    root / "website/rmlui.html",
    root / "website/build-and-ship.html",
    root / "website/troubleshooting.html",
    root / "website/limitations.html",
    root / "website/styles.css",
    root / "website/site.js",
    root / "website/search-index.js",
    ...
    root / "tools/validate-installed-workflow.py",
    root / "tools/check-release-version.py",
]
...
vulkan_renderer_source = (root / "engine/src/render/vulkan/vulkan_renderer.cpp").read_text(encoding="utf-8")
...
editor = "\n".join(path.read_text(encoding="utf-8") for path in sorted((root / "editor").glob("*")) if path.suffix in {".cpp", ".hpp"})
...
rc_test_runner = (root / "test-rc.ps1").read_text(encoding="utf-8")
behavioral_test_runner = (root / "test.ps1").read_text(encoding="utf-8")
engine_cmake = (root / "engine/CMakeLists.txt").read_text(encoding="utf-8")
root_cmake = (root / "CMakeLists.txt").read_text(encoding="utf-8")
_version_label_match = re.search(r'set\(VESPERA_VERSION_LABEL\s+"([^"]+)"\)', root_cmake)
if not _version_label_match:
    print("Could not read VESPERA_VERSION_LABEL from CMakeLists.txt")
    sys.exit(1)
version_label = _version_label_match.group(1)
...
gameplay_showcase = (root / "examples/reference_game/managed/GameplayShowcase.cs").read_text(encoding="utf-8")
...
checks = {
    "project version": "project(VesperaEngine VERSION 1.1.0" in root_cmake and f'set(VESPERA_VERSION_LABEL "{version_label}")' in root_cmake,
    ...
    "1.0 third-party notices": "SDL" in (root / "THIRD_PARTY_NOTICES.md").read_text(encoding="utf-8") and "Lua" in (root / "THIRD_PARTY_NOTICES.md").read_text(encoding="utf-8") and "RmlUi" in (root / "THIRD_PARTY_NOTICES.md").read_text(encoding="utf-8") and "FreeType" in (root / "THIRD_PARTY_NOTICES.md").read_text(encoding="utf-8") and "Dear ImGui" in (root / "THIRD_PARTY_NOTICES.md").read_text(encoding="utf-8") and "Vulkan-Headers" in (root / "THIRD_PARTY_NOTICES.md").read_text(encoding="utf-8"),
    "public limitations": "Windows x64" in (root / "docs/LIMITATIONS.md").read_text(encoding="utf-8") and "2D / UI Foundation" in (root / "docs/LIMITATIONS.md").read_text(encoding="utf-8") and "one active startup/runtime RML surface" in (root / "docs/LIMITATIONS.md").read_text(encoding="utf-8"),
    "practical documentation website": "Learn Vespera by doing" in (root / "website/index.html").read_text(encoding="utf-8") and "Build Game" in (root / "website/build-and-ship.html").read_text(encoding="utf-8") and "2D / UI Foundation" in (root / "website/two-d-foundation.html").read_text(encoding="utf-8") and "Search docs" in (root / "website/index.html").read_text(encoding="utf-8"),
    ...
    "editor entity filter": "ImGuiTextFilter entity_filter" in editor and 'Draw("Search entities##HierarchyFilter"' in editor,
    "editor tag layer inspector": (('InputText("Tag"' in editor and 'InputText("Layer"' in editor) or ('InputText("##EntityTag"' in editor and 'InputText("##EntityLayer"' in editor)),
    ...
    "editor transform inspector": 'Local Transform' in editor and (('DragFloat3("Rotation"' in editor and 'DragFloat3("Scale"' in editor) or ('DragFloat3("##EntityRotation"' in editor and 'DragFloat3("##EntityScale"' in editor)),
    ...
    "1.1 RC user test runner": "Automated RC gate" in rc_test_runner and "RESULT: PASS" in rc_test_runner and "RESULT: FAIL" in rc_test_runner and "Vespera-RC-Logs" in rc_test_runner and "D3D12 editor visual smoke" in rc_test_runner and "Vulkan reference-game visual smoke" in rc_test_runner and "tools\\run-rc-gate.ps1" in rc_test_runner and "& $Body | Out-Host" in rc_test_runner,
    "1.1 RC behavioral runner completeness": "--target vespera_engine_logic_tests" not in behavioral_test_runner and "& $CMake --build $BuildDir --config $Configuration" in behavioral_test_runner and "ctest-output.log" in behavioral_test_runner and "Full CTest output:" in behavioral_test_runner,
    "1.1 Release reference-game launcher": 'ValidateSet("Debug", "Release", "RelWithDebInfo")' in run_game and 'build\\examples\\reference_game\\$Configuration' in run_game and '-Configuration $Configuration' in run_game,
    "1.1 managed configuration propagation": '-Configuration $ManagedConfiguration' in (root / "run-project.ps1").read_text() and '-Configuration $ManagedConfiguration' in export_script and '-Configuration Release' in rc_gate and '[string]$Configuration = "Debug"' in managed_build_helper and '-c $Configuration' in managed_build_helper,
    ...
    "compact ImGui editor baseline": "VesperaWorkspaceToolbar" in editor and "VesperaWorkspaceStatus" in editor and "AssetSourceRail" in editor and 'draw_panel_heading("Asset"' in editor and "HierarchyHeader" in editor,
    ...
    "0.7.4 folder browsing": "asset_browser_folder" in editor and 'draw_panel_heading("Folders"' in editor,
    ...
    "0.7.5 editor asset workflow": "Source references remain linked when assets are moved in the Project panel." in editor and "Include in Standalone Build" in editor and "Build assets:" in editor,
    ...
    "0.7.6 editor stable-ref UX": "Asset ID:" in editor and "asset ID" in editor and "Saved source path is outdated" in editor,
    ...
    "0.7.7 editor move repair workflow": "Move / Rename Asset" in editor and "Repair Asset References" in editor and "repair_asset_fallback_paths" in editor,
    ...
    "0.7.7 managed asset dogfood": "WatcherPrefab" in gameplay_showcase and "ChimeAudio" in gameplay_showcase and "stable asset API prefab-id" in gameplay_showcase,
    ...
    "editor live 3D view": "struct SceneView3DState" in editor and ('ImGui::Begin("Scene")' in editor or 'ImGui::Begin("Scene",' in editor) and "draw_scene_view_3d(state)" in editor,
    ...
    "managed build integration": "vespera_builder.exe" in build_script and "--managed-only" in build_script and "$ManagedBuild" in run_game,
    ...
    "0.6 reference gameplay dogfood": "class GameplayShowcase" in gameplay_showcase and "Prefab.Instantiate" in gameplay_showcase and "Physics.Raycast2D" in gameplay_showcase and "SaveData" in gameplay_showcase,
    ...
    "0.6.1 deferred scene switching": "RequestSceneLoad" in managed_api and "take_scene_load_request" in managed_host and "load_runtime_scene" in reference and "alternate_chamber.slscene" in gameplay_showcase,
    ...
    "0.6.1 semantic managed property bridge": "GetPropertyFloat" in managed_api and "SetPropertyColor" in managed_api and "get_builtin_component_property" in managed_host and "typed SpriteRenderer animation/tint control active" in gameplay_showcase,
    "0.6.1 reference audio dogfood": "Audio.PlayOneShot" in gameplay_showcase and "audio_test" in gameplay_showcase and "test_chime.wav" in gameplay_showcase,
    ...
    "0.6.2 typed sprite animator": "public sealed class SpriteRenderer" in managed_api and "sprite.Play" in gameplay_showcase and "typed SpriteRenderer animation/tint control active" in gameplay_showcase,
    ...
    "0.6.2 reference spatial audio dogfood": "Audio.PlaySpatial" in gameplay_showcase and "audio_spatial" in gameplay_showcase and "lifecycle_test" in gameplay_showcase,
    "0.6.3 scene collection queries": "QueryEntities" in managed_api and "managed_query_entities" in managed_host and "FindAllWithTag" in managed_api and "FindAllOnLayer" in managed_api and "FindAllWithComponent" in managed_api and "collection queries" in gameplay_showcase,
    ...
    "0.6.3 typed collider light wrappers": "public sealed class CylinderCollider" in managed_api and "public sealed class PointLight" in managed_api and "EnsureCylinderCollider" in managed_api and "EnsurePointLight" in managed_api and "typed Collider + PointLight active" in gameplay_showcase,
    ...
    "0.7 project-aware managed build": "GameAssemblyName" in managed_build_helper and "--project $ProjectFile" in build_script and "--managed-only" in build_script,
...
checks.update({
    ...
    "0.9.8 MCP runtime-QA schemas": f'SERVER_VERSION = "{version_label}"' in mcp_bridge and '"name":"vespera_get_operation"' in mcp_bridge and '"name":"vespera_get_entity"' in mcp_bridge and '"name":"vespera_open_scene"' in mcp_bridge and '"name":"vespera_create_ui_document"' in mcp_bridge and '"name":"vespera_get_ui_layout"' in mcp_bridge and '"name":"vespera_export_project"' in mcp_bridge,
    ...
    "0.9.8 export script": f"Vespera Engine {version_label} export" in export_script and 'ValidateSet("Debug", "Development", "Release")' in export_script and 'ValidateSet("Project","FrameworkDependent","Portable")' in export_script.replace(" ", "") and "vespera_packager.exe" in export_script and "[switch]$Launch" in export_script,
    ...
    "public contributor QA guidance": (root / "CONTRIBUTING.md").exists() and "docs/TESTING.md" in (root / "CONTRIBUTING.md").read_text() and "docs/RELEASE_VALIDATION.md" in (root / "CONTRIBUTING.md").read_text(),
    ...
    "0.9.8 RmlUi production documentation": "production runtime UI path" in rmlui_doc and ".slui" in rmlui_doc and "Project Settings" in rmlui_doc and "UI.Find" in rmlui_doc,
    ...
    "0.9.3 managed UI ABI v8 additions": "UiGetNodeBoolCallback" in managed_host and "ui_set_node_float = managed_ui_set_node_float" in managed_host and "UiConsumeClick" in managed_api and "public float Value" in managed_api and "public bool Clicked" in managed_api and "Gameplay API: runtime UI control active" in gameplay_showcase,
...
checks.update({
    ...
    "0.9.9c managed UI dogfood": "UiSurface bridge active" in gameplay_showcase and "_uiProgress.ValueText = \"0.35\"" in gameplay_showcase,    "0.9.9d1 shared player target": "add_subdirectory(runtime/player)" in root_cmake and "add_executable(vespera_player" in player_cmake and "VesperaPlayerGame" in player_main and "discover_player_project" in player_main,
    ...
    "1.1.0 alpha2 project renderer runner": 'ValidateSet("auto", "d3d12", "vulkan", "null")' in run_project and "--renderer $Renderer" in run_project,
    ...
    "0.9.9e editor selected-project managed bootstrap": "Building project C# scripts before editor launch" in (root / "run.ps1").read_text() and "--managed-only" in (root / "run.ps1").read_text() and ".vespera\\managed" in (root / "run.ps1").read_text(),
    ...
    "0.9.9 in-editor build UX": 'ImGui::Begin("Build Game"' in editor and 'start_editor_build_job' in editor and 'Development + Run' in editor and 'Open Output Folder' in editor and 'Ctrl+Shift+B' in editor and 'editor_find_builder_executable' in editor,
    "0.9.9 background build log": 'std::future<int> future' in editor and '".vespera" / "logs"' in editor and 'Recent Build Output' in editor and 'run_process_to_file' in editor,
    ...
    "0.10.0c legacy slui authoring labeled": "Legacy .slui document. New project UI should use RML/RCSS." in editor,
    "release roadmap baseline": "current status: 1.1.0 — released" in (root / "docs/ROADMAP.md").read_text().lower() and "windows + direct3d 12 is the production baseline" in (root / "docs/ROADMAP.md").read_text().lower() and "0.10.0 final — scripting/ui convergence freeze" in (root / "docs/CHANGELOG.md").read_text().lower(),
})
...
checks.update({
    ...
    "0.13.0 actionable Build Game preflight": "Build Check Details" in editor and "Missing required build asset:" in editor and "Broken asset reference:" in editor and "saved build asset path is outdated" in editor,
...
checks.update({
    "1.1.0 renderer backend factory": (root / "engine/src/render/render_backend.cpp").exists() and "enum class RenderBackendType" in (root / "engine/include/vespera/render/render_backend.hpp").read_text() and "create_render_backend" in (root / "engine/src/render/render_backend.cpp").read_text() and "create_d3d12_render_backend" in renderer and "create_null_render_backend" in (root / "engine/src/render/null_renderer.cpp").read_text(),
    "1.1.0 player renderer selection": "--renderer <auto|d3d12|vulkan|null>" in player_main and "config.renderer = arguments->renderer" in player_main and "SDL_WINDOW_VULKAN" in application_source,
    "1.1.0 Linux compiler CI": (root / ".github/workflows/linux-ci.yml").exists() and "GCC logic tests" in (root / ".github/workflows/linux-ci.yml").read_text() and "Clang logic tests" in (root / ".github/workflows/linux-ci.yml").read_text(),
    "1.1.0 alpha14 Vulkan editor host": (root / "editor/editor_vulkan.cpp").exists() and "EditorVulkanHost" in (root / "editor/editor_vulkan.hpp").read_text() and "--renderer=vulkan" in editor and "VESPERA_EDITOR_VULKAN=1" in (root / "editor/CMakeLists.txt").read_text(),
    "1.1.0 alpha14 per-pixel Vulkan lighting": (root / "engine/shaders/vulkan/lighting_world.frag.spv").exists() and "VulkanLightingConstants" in vulkan_renderer_source and "upload_lighting_constants" in vulkan_renderer_source and "generate_vulkan_lighting_shaders.py" in (root / ".github/workflows/linux-ci.yml").read_text(),
    "1.1.0 alpha14 project-local managed output": 'state.project.root_directory / ".vespera" / "managed"' in editor and "managed_directory_" in play_runtime_h and "editor_user_settings_directory" in editor,
    "1.1.0 alpha14 prebuilt distribution": (root / "tools/package-engine.py").exists() and (root / "tools/validate-distribution.py").exists() and (root / "packaging/windows/VesperaEngine.iss").exists() and (root / ".github/workflows/release-assets.yml").exists() and "VesperaEngine-*-Windows-x64-Setup.exe" in (root / ".github/workflows/release-assets.yml").read_text(),
    "1.1.0 alpha17 installed workflow validation": (root / "tools/validate-installed-workflow.py").exists() and "Installed Vespera workflow validation: PASS" in (root / "tools/validate-installed-workflow.py").read_text() and "validate-installed-workflow.py" in (root / ".github/workflows/release-assets.yml").read_text() and "Smoke renderer stats: scene_passes=" in editor,
    "1.1.0 alpha17 release version guard": (root / "tools/check-release-version.py").exists() and "Release version mismatch" in (root / "tools/check-release-version.py").read_text() and "check-release-version.py" in (root / ".github/workflows/release-assets.yml").read_text(),
    "1.1.0 alpha19 release asset orchestrator": (root / "tools/build-release-assets.py").exists() and "Vespera Engine" in (root / "tools/build-release-assets.py").read_text() and "build-release-assets.py" in (root / ".github/workflows/release-assets.yml").read_text(),
    "1.1.0 alpha19 deterministic archives": "ARCHIVE_EPOCH" in (root / "tools/package-engine.py").read_text() and "ARCHIVE_ZIP_TIME" in (root / "tools/package-engine.py").read_text(),
    "1.1.0 alpha19 release tooling regression tests": (root / "tools/test-release-tooling.py").exists() and "Vespera release tooling tests: PASS" in (root / "tools/test-release-tooling.py").read_text() and "test-release-tooling.py" in (root / ".github/workflows/linux-ci.yml").read_text(),
    "1.1.0 alpha19 portable Windows CRT": "VESPERA_STATIC_MSVC_RUNTIME" in root_cmake and "-DVESPERA_STATIC_MSVC_RUNTIME=ON" in (root / "tools/build-release-assets.py").read_text(),
})

checks.update({
    "1.1.0 version surface": "project(VesperaEngine VERSION 1.1.0" in root_cmake and f'set(VESPERA_VERSION_LABEL "{version_label}")' in root_cmake and f'SERVER_VERSION = "{version_label}"' in mcp_bridge and f"Vespera Engine {version_label} export" in export_script,
    "1.1.0 Vulkan build wiring": 'option(VESPERA_VULKAN_RENDERER "Build the Vulkan renderer backend" ON)' in root_cmake and "Vulkan-Headers/archive/refs/tags/v1.4.357.tar.gz" in root_cmake and "src/render/vulkan/vulkan_renderer.cpp" in engine_cmake and "VESPERA_HAS_VULKAN=1" in engine_cmake and "VK_NO_PROTOTYPES=1" in engine_cmake,
    "1.1.0 Vulkan bootstrap core": "SDL_Vulkan_CreateSurface" in vulkan_renderer_source and "vkCreateSwapchainKHR" in vulkan_renderer_source and "vkAcquireNextImageKHR" in vulkan_renderer_source and "vkQueuePresentKHR" in vulkan_renderer_source,
    "1.1.0 alpha3 Vulkan geometry depth": "create_depth_resources" in vulkan_renderer_source and "vkCreateGraphicsPipelines" in vulkan_renderer_source and "vkCmdDrawIndexed" in vulkan_renderer_source and "upload_dynamic_geometry" in vulkan_renderer_source,
    "1.1.0 alpha4 reference game renderer selection": 'ValidateSet("auto", "d3d12", "vulkan", "null")' in (root / "run-game.ps1").read_text() and "config.renderer = requested_renderer" in (root / "examples/reference_game/main.cpp").read_text() and '"--renderer"' in (root / "examples/reference_game/main.cpp").read_text(),
    "1.1.0 alpha4 Vulkan world materials": "create_world_texture_array" in vulkan_renderer_source and "VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER" in vulkan_renderer_source and "vkCmdCopyBufferToImage" in vulkan_renderer_source and "vkCmdBindDescriptorSets" in vulkan_renderer_source and "upload_command_pool_" in vulkan_renderer_source and "OP_IMAGE_SAMPLE_IMPLICIT_LOD" in (root / "tools/generate_vulkan_bootstrap_shaders.py").read_text(),
    "1.1.0 alpha5 Vulkan runtime parity": "build_scene_geometry" in vulkan_renderer_source and "collect_active_lights" in vulkan_renderer_source and "resolve_sprite_frame" in vulkan_renderer_source and "create_ui_descriptor_resources" in vulkan_renderer_source and "ensure_ui_atlas" in vulkan_renderer_source and "sprite_pipeline_" in vulkan_renderer_source and "ui_pipeline_" in vulkan_renderer_source and "vkCmdDraw" in vulkan_renderer_source and "ui_draw_calls" in vulkan_renderer_source,
    "1.1.0 alpha6 Vulkan transfer parity": "VK_FORMAT_B8G8R8A8_UNORM" in vulkan_renderer_source and "VK_FORMAT_R8G8B8A8_UNORM" in vulkan_renderer_source and "extra automatic linear->sRGB encode" in vulkan_renderer_source and "Vulkan swapchain:" in vulkan_renderer_source,
    "1.1.0 alpha6 Linux managed host": "libhostfxr.so" in managed_host and "dlopen" in managed_host and "VESPERA_MANAGED_HOSTFXR" in managed_host and "${CMAKE_DL_LIBS}" in engine_cmake,
    "1.1.0 alpha6 finite graphical smoke mode": "VESPERA_SMOKE_FRAMES" in application_source and "VESPERA_SMOKE_FRAMES" in editor,
    "1.1.0 alpha6 Linux runtime staging": (root / "tools/build-managed-runtime.py").exists() and (root / "build-linux.sh").exists() and (root / "run-game-linux.sh").exists() and (root / "run-hub-linux.sh").exists(),
    "1.1.0 alpha6 Linux software Vulkan CI": "mesa-vulkan-drivers" in (root / ".github/workflows/linux-ci.yml").read_text() and "vulkaninfo --summary" in (root / ".github/workflows/linux-ci.yml").read_text() and "Smoke reference game through Vulkan + C# + Lua + UI" in (root / ".github/workflows/linux-ci.yml").read_text() and "xvfb-run" in (root / ".github/workflows/linux-ci.yml").read_text(),
    "1.1.0 alpha4 Vulkan reference CI": "vespera_reference_game" in (root / ".github/workflows/linux-ci.yml").read_text() and "VESPERA_BUILD_EXAMPLES=ON" in (root / ".github/workflows/linux-ci.yml").read_text(),
    "1.1.0 alpha3 bootstrap shaders": (root / "tools/generate_vulkan_bootstrap_shaders.py").exists() and (root / "engine/shaders/vulkan/bootstrap_world.vert.spv").exists() and (root / "engine/shaders/vulkan/bootstrap_world.frag.spv").exists() and (root / "engine/src/render/vulkan/vulkan_bootstrap_shaders.hpp").exists() and "spirv-val --target-env vulkan1.0" in (root / ".github/workflows/linux-ci.yml").read_text(),
    ...
    "1.0.0 final single-build release gate": "run-release-gate.ps1" in rc_gate and "RC public source hygiene" in rc_gate and "Staged public release hygiene" in rc_gate and "RC adversarial editor/runtime QA" in rc_gate and "--runtime-export-configuration" in rc_gate and '"Release"' in rc_gate and "PlayCycles" in rc_gate and "SceneSwitches" in rc_gate and "RC gate PASSED" in rc_gate and "__vespera_rc_qa_reference" in rc_gate and "RC post-QA source validation" in rc_gate and "Remove disposable RC QA project" in rc_gate and "RC post-QA staged public release hygiene" in rc_gate and "prepare-public-release.py" in rc_gate and "scale stress requires a clean saved edit scene" in (root / "tools/vespera_qa_runner.py").read_text(),
...
checks.update({
    ...
    "0.15.0 startup presentation": 'config.startup_splash_image = runtime_root / "branding" / "vespera_splash.png"' in (root / "examples/project_hub/main.cpp").read_text() and 'config.startup_sound = runtime_root / "branding" / "vespera_logo_sting.wav"' in (root / "examples/project_hub/main.cpp").read_text() and "startup_splash_minimum_seconds" in application_source and "kEditorStartupSplashMinimumSeconds" in editor,
...
checks.update({
    ...
    "0.12.0 serialized identifier allowlist": '"sectorline.transform"' in (root / "engine/include/vespera/scene/scene.hpp").read_text() and "sectorline_scene 15" in (root / "docs/SCENE_FORMAT.md").read_text() and 'output << "sectorline_scene "' in (root / "engine/src/scene/scene_io.cpp").read_text(),
})

current_failed = [name for name, ok in checks.items() if name.startswith(("0.9.8", "0.9.9", "0.12.0", "0.13.0", "0.14.0", "0.14.1", "0.15.0", "0.15.1", "0.15.2", "0.16.0", "0.16.2", "1.0.0", "1.1.0")) and not ok]
if current_failed:
...
print(f"Vespera Engine {version_label} source structure validation passed.")
